  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        // Si tiene patrón de recurrencia, cargamos la recurrente completa
        if (normal.recurrencePattern) {
          const recurring = await getRecurringTransactionById(id);
          setForm({
            ...recurring,
            // asegurar que siempre haya categoryId plano
            categoryId:
              recurring.category?.categoryId ??
              recurring.categoryId ??
              "",
            // asegurar tripId plano si viene anidado
            tripId: recurring.trip?.tripId ?? recurring.tripId ?? "",
          });
          setIsRecurring(true);
        } else {
          // Transacción normal: ahora el DTO ya trae categoryId
          setForm({
            ...normal,
            categoryId: normal.categoryId ?? "",
            tripId: normal.tripId ?? normal.trip?.tripId ?? "",
          });
          setIsRecurring(false);
        }
      } catch (err) {
        console.error(err);
        toast.error(t("notFound", { defaultValue: "Transaction not found" }));
        navigate("/transactions");
      }
    }

    if (id && token) fetchData();
  }, [id, token, userId, navigate, t]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        categoryId: form.categoryId || "", // <-- siempre ID plano
        tripId: form.tripId || form.trip?.tripId || null,
      };
      ...
      toast.success(t("updateSuccess", { defaultValue: "Transaction updated successfully" }));
      navigate("/transactions");
    } catch (err) {
      console.error(err);
      toast.error(t("updateError", { defaultValue: "Failed to update transaction" }));
    } finally {
      setSaving(false);
    }
  };

  if (!form) return <div className="p-6">{t("loading", { defaultValue: "Loading..." })}</div>;
      ...
      <form onSubmit={handleSubmit} className="space-y-4 bg-white p-6 shadow rounded-xl">
        ...
        {trips.length > 0 && !isRecurring && (
          <div>
            ...
            <select
              name="tripId"
              value={form.tripId || form.trip?.tripId || ""}
              onChange={handleChange}
              className="w-full p-2 border rounded"
            >
